...
from diagmind.language_model import parse  # noqa: E402
...
layout_planner_prompt = load_prompt("layout_planner.txt")
...
# Generate layout plan using layout planner
print("\n" + "="*60)
print("Generating layout plan...")
print("="*60)

layout_planner_user_prompt = f"""Topic: {inputs['topic']}
Graph Structure:
{graph}

Convert this graph into a natural, textbook-style illustration plan.
Output drawing instructions describing the visual objects and their spatial placement on the canvas."""

layout_plan_response = parse(
    system_prompt=layout_planner_prompt,
    user_prompt=layout_planner_user_prompt,
    response_format=LayoutPlanResponse,
)

print(f"\nGenerated {len(layout_plan_response.instructions)} drawing instructions:")
for i, instruction in enumerate(layout_plan_response.instructions, 1):
    print(f"  {i}. {instruction}")

# Save layout plan to JSON
...
with open(layout_output_file, 'w') as f:
    json.dump(layout_plan_response.model_dump(), f, indent=2)
